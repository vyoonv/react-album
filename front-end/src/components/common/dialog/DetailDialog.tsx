    ...
    if (isBookmarked) {
      try {
        const response = await fetch(
          `http://localhost:80/bookmark/${selected.id}`,
          {
            method: "DELETE",
          }
        );
      ...
      try {
        const { id, urls, user, width, height, created_at, updated_at, likes } =
          selected;
          ...
          <div className={styles.bookmark}>
            ...
            >
              <span
                ...
                style={{
                  fontSize: "16px",
                  color: bookmarks.some((item) => item.imageId === data.id)
                    ? "red"
                    : "black",
                }}
              >
